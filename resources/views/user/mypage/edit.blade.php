...
@section('content')
    <form action="{{ route('mypage.update', ['id' => $kachi->id ]) }}" method="POST" class="p-5">
    @csrf
    @if ($errors->any())
    ...
    @endif
      <div class="w-screen flex flex-col justify-center items-center">
        ...
        <div class="w-3/4 mt-4">
          <textarea name="kachi" class="block w-full h-32 px-4 py-2 border-none resize-none text-gray-700 bg-white border rounded-md focus:outline-none focus:ring" placeholder="理想の部屋、暮らし方をイメージして書いてみましょう">{{ $kachi->kachi }}</textarea>
        </div>
