            ...
            <div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
                    ...
                    @foreach($listbuys as $listbuy)
                        <tr>
                            ...
                            <div class="flex">
                                <div>
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        <a class="text-blue-400" href="{{ route('buy_list.show', ['id' => $listbuy->id]) }}">
                                ...
                                <div>
                                    <p class="text-gray-900 whitespace-no-wrap my-1 ml-1">{{ $listbuy->wantbuys->where('delete_status', '=', 0)->count() }}</p>
                                </div>
                            </div>
                            </td>
                            
                            <td class="px-5 py-5 text-sm text-right bg-stone-200 border-b border-gray-200">
                            @if(!($listbuy->list_name == "未分類"))
                                <form id="" method="post" action="{{ route('buy_list.destroy', ['id'=> $listbuy->id]) }}">
                                @csrf
                                    <p class="text-gray-900 whitespace-no-wrap">
                ...
                <div class="flex justify-center py-4">
                    <button class="flex items-center px-4 py-2 font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-green-500 rounded-lg hover:bg-green-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-80">
                        ...
                        <a href="{{ route('buy_list.create') }}">新規リスト作成</a>
                    </button>
                </div>
                <table class="min-w-full leading-normal">
                    <thead>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <div class="flex">
                                <div>
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        <a class="text-blue-400" href="{{ route('buy_list.buy') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                                            </svg>
                                        </a>
                                ...
                                <div>
                                    <p class="text-gray-900 whitespace-no-wrap my-1 ml-1">{{ $wantbuy_count }}</p>
                                </div>
                            </div>
                            </td>
                            <td class="px-5 py-5 text-sm bg-slate-200 border-b border-gray-200">
